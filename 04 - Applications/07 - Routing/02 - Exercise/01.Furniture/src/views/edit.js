...
export async function editPage(ctx) {
    ...
    async function onSubmit(ev) {
        ...
        const editedItem = [...formData.entries()].reduce((a, [k, v]) => Object.assign(a, { [k]: v }), {});

        if (Object.entries(editedItem).filter(([k, v]) => k != 'material').some(([k, v]) => v == '')) {
            return alert('Please fill in all mandatory fields!');
        }

        editedItem.year = Number(editedItem.year);
        editedItem.price = Number(editedItem.price);

        await editFurniture(id, editedItem);
        ctx.page.redirect('/');
    }
}