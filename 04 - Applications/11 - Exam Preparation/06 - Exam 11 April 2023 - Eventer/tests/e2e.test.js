...
describe("E2E tests", function () {
  // Setup
  this.timeout(DEBUG ? 90000 : 90000);
  before(
    async () =>
  ...
  describe("Events Page [ 15 Points ]", () => {
    ...
    it("Check Events page Info [ 2.5 Points ]", async () => {
      const { get } = await handle(endpoints.catalog);
  ...
  describe("BONUS : Going functionality  [ 15 Points ]", async () => {
    ...
    it("Going button should increase total buys by 1 after a click on it [ 2.5 Points ]", async () => {
      // Login user
      const user = mockData.users[0];
      const data = mockData.catalog[2];
      await page.goto(host);
